...
Every lab must package a `config.json` inside its `/app` directory. This file dictates how the Workspace interface interacts with the lab. Below is the standard structure:
...
### Lab Requirements
1. **Directory Structure:** Inside the Docker image, code must reside under `/app`.
2. **Main Entrypoint:** Must include `/app/main.py` which runs the web application.
...
```json
{
  ...
  "port": 5002,
  "download_url": "http://host.docker.internal:4444/cmdi-0.tar"
}
```
